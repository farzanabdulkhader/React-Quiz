...
app.get("/highscore", (req, res) =>
  res.json({ highscore: HIGHSCORE[0].highscore })
);

app.put("/highscore", (req, res) => {
  const { highscore } = req.body;

  if (typeof highscore === "number" && highscore > HIGHSCORE[0].highscore) {
    HIGHSCORE[0].highscore = highscore;
    return res.json({ highscore: HIGHSCORE[0].highscore });
  }
  res.status(400).json({ message: "Invalid highscore" });
});
